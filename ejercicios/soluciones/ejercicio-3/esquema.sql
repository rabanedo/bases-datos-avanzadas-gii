...
CREATE TABLE medicos (
  ...
  fecha_alta        DATE,
  CONSTRAINT chk_medicos_especialidad CHECK (LOWER(especialidad) IN ('cardiología', 'fisioterapia', 'ginecología', 'urología', 'traumatología'))
);
...
CREATE TABLE consultas (
  ...
  motivo            VARCHAR(20),
  CONSTRAINT        chk_consultas_motivo CHECK (LOWER(motivo) IN ('Consulta', 'Revisión', 'Análisis clínicos', 'Rehabilitación'))
);
...
CREATE TABLE pagos (
  ...
  CONSTRAINT chk_pagos_metodo CHECK (LOWER(metodo) IN ('tarjeta', 'paypal', 'transferencia', 'bizum')),
  CONSTRAINT chk_pagos_estado CHECK (LOWER(estado) IN ('pendiente', 'pagado', 'fallido', 'reembolsado'))
);
